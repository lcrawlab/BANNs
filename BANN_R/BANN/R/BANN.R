#' BANN main function
#' @X is an n-by-p genotype matrix where n is the number of samples and p is the number of input variables (e.g. SNPs).
#' @y is the phenotypic vector of length n.
#' @mask is the p-by-g annotation file where g is the number of groups (e.g., SNP-sets).
#' @centered is used to check whether X is normalized.
#' @tol is the tolerance for checking convergence.
#' @maxiter is the maximum iteration for updating parameters.
#' @show_progress is the indicator for whether the function should print out the progress of the model fitting.
#' @examples
#' res = BANN(X, mask, y)
