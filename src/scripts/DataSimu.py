...
X=np.genfromtxt("/Users/pinardemetci/Desktop/BANNs/src/python/Simulation_X.csv", delimiter=",")
n=X.shape[0]
p=X.shape[1]
...
rho= 1 #75% of H^2 is explained by additive effects
percent=1
...
for sim in range(0,100):
	...
	y_filename="Simu/06H1r1p/y_"+str(sim)+".txt"
	snp_filename="Simu/06H1r1p/snp_"+str(sim)+".csv"
	gene_filename="Simu/06H1r1p/gene_"+str(sim)+".csv"
